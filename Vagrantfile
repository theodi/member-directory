...
#    end
  ...
  config.vm.define :directory_theodi_org_03 do |dto_03_config|
    dto_03_config.vm.box      = "dummy"
    dto_03_config.vm.hostname = "directory-03"

    dto_03_config.ssh.private_key_path = "./.chef/id_rsa"
    dto_03_config.ssh.username         = "root"

    dto_03_config.vm.provider :rackspace do |rs|
      rs.username        = y["username"]
      rs.api_key         = y["api_key"]
      rs.flavor          = /2GB/
      rs.image           = /Precise/
    ...
    dto_03_config.vm.provision :shell, :inline => "curl -L https://www.opscode.com/chef/install.sh | bash"

    dto_03_config.vm.provision :chef_client do |chef|
      chef.node_name              = "directory-03"
      chef.environment            = "production"
      chef.chef_server_url        = "https://chef.theodi.org"
      chef.validation_client_name = "chef-validator"
      chef.validation_key_path    = ".chef/chef-validator.pem"
